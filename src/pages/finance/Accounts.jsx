...
export default function FinanceAccounts() {
  ...
  const handleDelete = async (type, id, name) => {
    ...
    try {
      if (type === 'account') await financeService.deleteAccount(id);
      else if (type === 'invoice') await financeService.deleteInvoice(id);
  ...
  const handleConfirmPayment = async () => {
    ...
    setProcessing(true);
    try {
      await financeService.markInvoiceAsPaid(paymentModal.invoice._id, {
  ...
  const handleSendEmail = async (invoice) => {
    ...
    try {
      await financeService.sendInvoiceEmail(invoice._id, invoice);
  ...
  const handleSendWhatsApp = async (invoice) => {
    ...
    try {
      await financeService.sendInvoiceWhatsApp(invoice._id, invoice);
  ...
  const tabs = [
    { id: 'accounts', name: 'Chart of Accounts', icon: HiCash },
    ...
    <div className="space-y-6">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y">
                {filteredAccounts.length === 0 ? (
                  <tr><td colSpan="6" className="px-4 py-8 text-center text-gray-500">No accounts found</td></tr>
                ) : (
                  filteredAccounts.map(a => (
                    <tr key={a._id} className="hover:bg-gray-50">
                      <td className="px-4 py-3 font-medium">{a.name}</td>
                      <td className="px-4 py-3 capitalize">{a.type}</td>
                      <td className="px-4 py-3">{a.currency || 'KES'}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex justify-center space-x-2">
                          <button onClick={() => handleViewDetails('account', a)} className="text-blue-600 hover:text-blue-800" title="View">
                            <HiEye className="h-4 w-4" />
                          </button>
                          <button onClick={() => handlePrint('account', a)} className="text-purple-600 hover:text-purple-800" title="Print Statement">
                            <HiPrinter className="h-4 w-4" />
                          </button>
                          <button onClick={() => setModal({ type: 'account', data: a })} className="text-green-600 hover:text-green-800" title="Edit">
                            <HiPencil className="h-4 w-4" />
                          </button>
                          <button onClick={() => handleDelete('account', a._id, a.name)} className="text-red-600 hover:text-red-800" title="Delete">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y">
                {filteredWithdrawals.length === 0 ? (
                  <tr><td colSpan="7" className="px-4 py-8 text-center text-gray-500">No withdrawals found</td></tr>
                ) : (
                  filteredWithdrawals.map(w => (
                    <tr key={w._id} className="hover:bg-gray-50">
                      <td className="px-4 py-3">{formatDate(w.date)}</td>
                      <td className="px-4 py-3 font-mono text-xs">{w.reference || '-'}</td>
                      <td className="px-4 py-3">{w.description}</td>
                      <td className="px-4 py-3">{w.accountId?.name || '-'}</td>
                      <td className="px-4 py-3 capitalize">{w.paymentMethod}</td>
                      <td className="px-4 py-3 text-right font-semibold text-red-600">{formatCurrency(w.amount)}</td>
                      <td className="px-4 py-3">
                        <div className="flex justify-center space-x-2">
                          <button onClick={() => handleViewDetails('withdrawal', w)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded" title="View">
                            <HiEye className="h-4 w-4" />
                          </button>
                          <button onClick={() => handlePrint('withdrawal', w)} className="p-1.5 text-purple-600 hover:bg-purple-50 rounded" title="Print">
                            <HiPrinter className="h-4 w-4" />
                          </button>
                        </div>
                      </td>
        ...
        {paymentModal.invoice && (
          <div className="space-y-4">
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <p className="text-2xl font-bold text-purple-600 mt-2">{formatCurrency(paymentModal.invoice.total)}</p>
            </div>
      ...
      <Modal isOpen={viewDetails} onClose={() => { setViewDetails(false); setSelectedItem(null); }} title={selectedItem?.type === 'invoice' ? 'Invoice Details' : selectedItem?.type === 'withdrawal' ? 'Withdrawal Details' : 'Account Details'} size="lg">
        {selectedItem && selectedItem.type === 'account' && (
          <div className="space-y-4">
            <div className="bg-gray-50 p-4 rounded-lg"><h3 className="font-semibold text-lg">{selectedItem.name}</h3><p className="text-gray-600">{selectedItem.description || 'No description'}</p></div>
            <div className="grid grid-cols-3 gap-4"><div><p className="text-sm text-gray-500">Type</p><p className="font-medium capitalize">{selectedItem.type}</p></div><div><p className="text-sm text-gray-500">Currency</p><p className="font-medium">{selectedItem.currency}</p></div><div><p className="text-sm text-gray-500">Status</p><span className={`px-2 py-1 text-xs rounded-full ${selectedItem.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>{selectedItem.isActive ? 'Active' : 'Inactive'}</span></div></div>
            <div className="bg-blue-50 p-4 rounded-lg text-center"><p className="text-sm text-blue-600">Current Balance</p><p className="text-3xl font-bold text-blue-600">{formatCurrency(selectedItem.balance, selectedItem.currency)}</p></div>
            <div className="flex justify-end space-x-3 pt-4"><Button variant="outline" onClick={() => handlePrint('account', selectedItem)}><HiPrinter className="mr-2" />Print Statement</Button><Button onClick={() => { setViewDetails(false); setSelectedItem(null); }}>Close</Button></div>
          </div>
        )}
        {selectedItem && selectedItem.type === 'invoice' && (
          <div className="space-y-4">
            <div className="bg-gray-50 p-4 rounded-lg"><div className="flex justify-between"><div><p className="text-sm text-gray-500">Invoice #</p><p className="font-mono font-medium">{selectedItem.invoiceNumber}</p></div><div className="text-right"><p className="text-sm text-gray-500">Status</p><span className={`px-2 py-1 text-xs rounded-full ${getStatusBadge(selectedItem.status)}`}>{selectedItem.status}</span></div></div></div>
            <div className="grid grid-cols-2 gap-4"><div><p className="text-sm text-gray-500">Customer</p><p className="font-medium">{selectedItem.customerName}</p></div><div><p className="text-sm text-gray-500">Email</p><p className="font-medium">{selectedItem.customerEmail || 'N/A'}</p></div><div><p className="text-sm text-gray-500">Invoice Date</p><p className="font-medium">{formatDate(selectedItem.invoiceDate)}</p></div><div><p className="text-sm text-gray-500">Due Date</p><p className="font-medium">{formatDate(selectedItem.dueDate)}</p></div></div>
            <div><p className="text-sm text-gray-500 mb-2">Items</p><table className="w-full text-sm"><thead className="bg-gray-100"><tr><th className="p-2 text-left">Item</th><th className="p-2 text-right">Qty</th><th className="p-2 text-right">Price</th><th className="p-2 text-right">Total</th></tr></thead><tbody>{selectedItem.items?.map((item, i) => (<tr key={i} className="border-b"><td className="p-2">{item.description}</td><td className="p-2 text-right">{item.quantity}</td><td className="p-2 text-right">{formatCurrency(item.unitPrice)}</td><td className="p-2 text-right">{formatCurrency(item.total)}</td></tr>))}</tbody></table></div>
            <div className="border-t pt-3"><div className="flex justify-between"><span>Subtotal</span><span>{formatCurrency(selectedItem.subtotal)}</span></div>{selectedItem.tax > 0 && <div className="flex justify-between"><span>Tax</span><span>{formatCurrency(selectedItem.tax)}</span></div>}{selectedItem.discount > 0 && <div className="flex justify-between"><span>Discount</span><span>-{formatCurrency(selectedItem.discount)}</span></div>}<div className="flex justify-between font-bold text-lg mt-2"><span>Total</span><span>{formatCurrency(selectedItem.total)}</span></div></div>
            <div className="flex justify-end space-x-3 pt-4"><Button variant="outline" onClick={() => handlePrint('invoice', selectedItem)}><HiPrinter className="mr-2" />Print</Button><Button onClick={() => { setViewDetails(false); setSelectedItem(null); }}>Close</Button></div>
          </div>
        )}
        {selectedItem && selectedItem.type === 'withdrawal' && (
          <div className="space-y-4">
            <div className="bg-gray-50 p-4 rounded-lg"><p className="text-sm text-gray-500">Reference</p><p className="font-medium">{selectedItem.reference || 'N/A'}</p></div>
            <div className="grid grid-cols-2 gap-4"><div><p className="text-sm text-gray-500">Date</p><p className="font-medium">{formatDate(selectedItem.date)}</p></div><div><p className="text-sm text-gray-500">Account</p><p className="font-medium">{selectedItem.accountId?.name || 'N/A'}</p></div><div><p className="text-sm text-gray-500">Method</p><p className="font-medium capitalize">{selectedItem.paymentMethod}</p></div></div>
            <div><p className="text-sm text-gray-500">Description</p><p className="font-medium">{selectedItem.description}</p></div>
            {selectedItem.notes && <div><p className="text-sm text-gray-500">Notes</p><p>{selectedItem.notes}</p></div>}
            <div className="bg-red-50 p-4 rounded-lg text-center"><p className="text-sm text-red-600">Amount Withdrawn</p><p className="text-2xl font-bold text-red-600">{formatCurrency(selectedItem.amount)}</p></div>
            <div className="flex justify-end space-x-3 pt-4"><Button variant="outline" onClick={() => handlePrint('withdrawal', selectedItem)}><HiPrinter className="mr-2" />Print</Button><Button onClick={() => { setViewDetails(false); setSelectedItem(null); }}>Close</Button></div>
          </div>
        )}
      </Modal>
    </div>
