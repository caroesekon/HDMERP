...
import {
  HiHome, HiUserGroup, HiClock, HiCash, HiCalendar, HiStar, HiOfficeBuilding,
  HiMenu, HiX, HiChevronRight, HiCog
} from 'react-icons/hi';
...
const navItems = [
  { name: 'Dashboard', href: '/staff', icon: HiHome },
  { name: 'Employees', href: '/staff/employees', icon: HiUserGroup },
  { name: 'Departments', href: '/staff/departments', icon: HiOfficeBuilding },
  { name: 'Attendance', href: '/staff/attendance', icon: HiClock },
  { name: 'Payroll', href: '/staff/payroll', icon: HiCash },
  { name: 'Leave', href: '/staff/leave', icon: HiCalendar },
  { name: 'Performance', href: '/staff/performance', icon: HiStar },
  { name: 'Settings', href: '/staff/settings', icon: HiCog },
];
...
export default function StaffLayout() {
  ...
  const { appName, logo } = useModuleSettings('staff', 'Staff Manager');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header */}
      <header className="bg-gradient-to-r from-green-600 to-green-700 shadow-md sticky top-0 z-30">
        <div className="px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center">
              <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 -ml-2 rounded-md text-white hover:bg-green-800">
                <HiMenu className="h-6 w-6" />
              </button>
              <div className="flex items-center ml-2 lg:ml-0">
                {logo ? <img src={logo} alt={appName} className="h-8 w-auto" /> : <HiUserGroup className="h-8 w-8 text-white" />}
                <div className="ml-3">
                  <h1 className="text-lg sm:text-xl font-bold text-white leading-tight">{appName}</h1>
                  <p className="text-xs text-green-100 hidden sm:block">Employee & HR Management</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-2 sm:space-x-4">
              <div className="hidden md:block text-right mr-4">
                ...
                <div className="text-green-100 text-xs">{formatDate(currentTime)}</div>
              </div>
              <Link to="/" className="text-xs sm:text-sm text-white/80 hover:text-white px-2 sm:px-3 py-2 rounded-lg hover:bg-green-800 transition">
                Hub
              </Link>
        ...
        <aside className="hidden lg:block w-64 bg-white shadow-md min-h-[calc(100vh-4rem)] sticky top-16">
          <nav className="p-4 space-y-1">
            {navItems.map((item) => {
              ...
              return (
                <Link key={item.name} to={item.href} className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-colors ${isActive ? 'bg-green-50 text-green-700 font-medium' : 'text-gray-700 hover:bg-gray-50'}`}>
            ...
            <div className="flex flex-col h-full">
              <div className="p-4 border-b border-gray-200 bg-gradient-to-r from-green-600 to-green-700">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    {logo ? <img src={logo} alt={appName} className="h-6 w-auto" /> : <HiUserGroup className="h-6 w-6 text-white" />}
                    <div className="ml-2">
                      <h2 className="text-lg font-semibold text-white">{appName}</h2>
                      <p className="text-xs text-green-100">Employee & HR Management</p>
                    </div>
                  </div>
              ...
              <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
                {navItems.map((item) => {
                  const isActive = location.pathname === item.href || (item.href !== '/staff' && location.pathname.startsWith(item.href));
                  return (
                    <Link key={item.name} to={item.href} className={`flex items-center justify-between px-4 py-3 rounded-lg ${isActive ? 'bg-green-50 text-green-700 font-medium' : 'text-gray-700 hover:bg-gray-50'}`}>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-sm border-t border-gray-200 shadow-lg z-20">
        <div className="grid grid-cols-5 h-16">
          {navItems.slice(0, 5).map((item) => {
            const isActive = location.pathname === item.href || (item.href !== '/staff' && location.pathname.startsWith(item.href));
            return (
              <Link key={item.name} to={item.href} className={`flex flex-col items-center justify-center ${isActive ? 'text-green-600' : 'text-gray-500'}`}>
